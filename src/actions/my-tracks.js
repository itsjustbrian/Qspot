...
import { userSelector, userIdSelector } from '../reducers/auth.js';
...
const attachMyTracksListener = () => (dispatch, getState) => {
  return tracksPromise || (tracksPromise = new Promise((resolve) => {
    ...
    tracksListener = firestore.collection(`parties/${currentParty}/tracks`).where('submitterId', '==', currentUserId).orderBy('timestamp').onSnapshot((snapshot) => {
